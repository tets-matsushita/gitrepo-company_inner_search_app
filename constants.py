...
SUPPORTED_EXTENSIONS = {
    ...
    # 追加: .txt を読み込む（UTF-8、必要なら cp932 で再試行する処理は initialize側で行う）
...
"""
# 追加: マジックナンバーを定数化
RETRIEVER_K = 5              # ベクターストアから取り出す関連ドキュメント数
CHUNK_SIZE = 500             # ドキュメントを分割するチャンクサイズ
CHUNK_OVERLAP = 50           # チャンク分割時のオーバーラップ量